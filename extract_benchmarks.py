...
SCE_SEEDS = ['0', '1', '2', '3', '4', '5', '6', '7'] # yhwang 06/2024
...
def _read_gauge_info(path):
    ...
    with open(path) as gauge_info_file:
        next(gauge_info_file) # yhwang 20240603 skip the title row
...
# load basin list
with open('analysis/results_data/global_mclstm_static.pkl', 'rb') as f:
...
# --- SAC-SMA Model ----------------------------------------------------
# Use the Sac-SMA results from Frame et al. (2022), where the forcing is NLDAS. 
# https://www.hydroshare.org/resource/d750278db868447dbd252a8c5431affd/
# by yhwang 06/2024
...
for basin in basin_list:
    ...
    for sdex in range(len(SCE_SEEDS)):
        ...
        # load this seed's data and store in a basin-specific dataframe
        idx = np.where(meta_df['gauge_str'].values == basin)[0][0]
        fname = f"analysis/results_data/sacsma_standard_training_results/config_{SCE_SEEDS[sdex]}/{basin}.pkl"
        with open(fname, 'rb') as file:
            data = pickle.load(file) 
            temp_df = pd.DataFrame(data[1], columns=[f'qsim_{sdex}'])
        basin_df = pd.merge(
            basin_df, temp_df[f"qsim_{sdex}"], how='inner', left_index=True, right_index=True)
